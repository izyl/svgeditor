...
(function(R) {
	...
	function tag(name, attrs, matrix, content) {
		...
		return '<' + name + (matrix ? ' transform="matrix(' + matrix.toString().replace(/^matrix\(|\)$/g, '') + ')" ' : ' ') + attrs + '>' + content + '</'
				+ name + '>';
	}
	/**
	...
	var serializer = {
		'text' : function(node) {
			var style = extractStyle(node), tags = new Array, textLines = node.attrs['text'].toString().split('\n'), totalLines = textLines.length;
		...
		for (var node = paper.bottom; node != null; node = node.next) {
			...
			for (i in node.attrs) {
				...
				case 'gradient':
					...
					var fx = 0.5, fy = 0.5;
					gradient = String(gradient).replace(
							R._radial_gradient,
							function(all, _fx, _fy) {
								...
								if (_fx && _fy) {
									...
									fy = parseFloat(_fy);
									var dir = ((fy > .5) * 2 - 1);
									Math.pow(fx - .5, 2) + Math.pow(fy - .5, 2) > .25 && (fy = Math.sqrt(.25 - Math.pow(fx - .5, 2)) * dir + .5) && fy != .5
					...
					// assuming gradient is validated already!!
						...
						var vector = [ 0, 0, Math.cos(R.rad(angle)), Math.sin(R.rad(angle)) ], max = 1 / (Math.max(Math.abs(vector[2]), Math.abs(vector[3])) || 1);
						vector[2] *= max;
						vector[3] *= max;
						if (vector[2] < 0) {
							vector[0] = -vector[2];
						...
						svg += '<defs>';
						svg += ' <linearGradient id="lineargradient' + id + '" x1="' + vector[0] + '" y1="' + vector[1] + '" x2="' + vector[2] + '" y2="'
								+ vector[3] + '">';
						for (var di = 0; di < dots.length; di++) {
							var offset = (di / (dots.length - 1) * 100) + '%';
							// if dot has an offset
